...
env_name = 'HandManipulateBlock-v1'
env = gym.make(env_name)
env = Monitor(env)
env2 = gym.make("HandReach-v1")

model = TQC.load("./model/HandReach.pkl", env=env2)
model2 = TQC(
    env=env,
    ...
    tensorboard_log="./tensorboard/HandReachToBlock/"
)

original_first_layer = [model.critic.q_networks[0][0], model.critic.q_networks[1][0]]
model2.critic.q_networks[0][0].weight.data[:,:48] = original_first_layer[0].weight.data[:,:48]
model2.critic.q_networks[1][0].weight.data[:,:48] = original_first_layer[1].weight.data[:,:48]
...
model2.save("./model/HandReachToBlock.pkl")
